...
- **Network:** `devnet`
- **Package ID:** `DEPLOY_CONTRACT_FIRST`
...
public fun increment(manager: &mut Bookmarkmanager) {
manager.value = manager.value + 1;
}
public fun set_value(manager: &mut Bookmarkmanager, value: u64, ctx: &TxContext) {
...
- Use Package ID: `DEPLOY_CONTRACT_FIRST` and Module: `contract`
